...
- runtimes: JVM `clojure -M -e '(load-file "evidence/falsify13-insurance-accum.cljc") (load-file "evidence/falsify13-jvm-driver.cljc")'` → falsify13-jvm.{out,err} (exit=0); nbb `nbb --classpath "$(nbb --classpath "../text/src" script/nbb-classpath.cljs)" evidence/falsify13-nbb-driver.cljs` → falsify13-nbb.{out,err} (exit=0)。nbb classpath bootstrap は frame-394 発見の自己依存 fix (`--classpath ../text/src` 抜き) を使用。
