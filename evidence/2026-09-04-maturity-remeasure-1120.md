...
2. **nbb tests**: `nbb --classpath "$(nbb script/nbb-classpath.cljk)" script/tests-on-nbb.cljk` → evidence/test-nbb-1113.out: **same count 357/915, 0 failures, 0 errors. TESTS-ON-NBB: pass.** exit=0. (32nd identical-count measurement.)
3. **Seeded fuzz JVM**: `clojure -M -e "(load-file \"evidence/fuzz-seeded.cljk\") (fuzz-seeded/run)"` → evidence/fuzz-jvm-1114.out, 16 seeds, done. (Note: `(torihiki.fuzz-seeded/run)` fully-qualified form fails with ClassNotFoundException; bare `(fuzz-seeded/run)` after load-file is correct.)
4. **Seeded fuzz nbb**: `nbb --classpath "$(nbb script/nbb-classpath.cljk)" evidence/fuzz-nbb-driver.cljk` → evidence/fuzz-nbb-1118.out, 16 seeds, done.
