...
- runtimes: JVM `kbb -M -e '(load-file "evidence/falsify8-halt-paths.cljk") (load-file "evidence/falsify8-jvm-driver.cljk")'` → falsify8-jvm.{out,err} (exit=0); nbb `kbb --backend sci --classpath "$(kbb --backend sci script/nbb-classpath.cljk)" evidence/falsify8-driver.cljk` → falsify8-nbb.{out,err} (exit=0)
