...
- Trigger condition: `kbb -M:test` (+ optional `kbb -M:bench`) with results recorded under evidence/.
...
Next iteration: re-check load; if < 20, run `kbb -M:test` and `kbb -M:bench` normally.
