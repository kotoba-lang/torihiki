...
- nbb `nbb --classpath "$(nbb script/nbb-classpath.cljs)" script/tests-on-nbb.cljs`:
  ...
  (evidence/test-nbb-0156.out) — 16th identical-count run. Note: bare `nbb script/tests-on-nbb.cljs`
  fails with "Could not find namespace: torihiki.address-test"; classpath via script/nbb-classpath.cljs is required, as documented.
