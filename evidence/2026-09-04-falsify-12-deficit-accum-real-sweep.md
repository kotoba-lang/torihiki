...
- runtimes: JVM `kbb -M -e '(load-file "evidence/falsify12-multi-deficit.cljk") (load-file "evidence/falsify12-jvm-driver.cljk")'` → falsify12-jvm.{out,err} (exit=0); nbb `kbb --backend sci --classpath "$(kbb --backend sci script/nbb-classpath.cljk)" evidence/falsify12-driver.cljk` → falsify12-nbb.{out,err} (exit=0)。両 stdout は REPL echo 行を除き diff で比較。
