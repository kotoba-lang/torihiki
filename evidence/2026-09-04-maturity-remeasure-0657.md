...
- **テスト両 runtime 実測 29 度目**: JVM `clojure -M:test` (evidence/test-0657.out, JVM_EXIT=0) と nbb `nbb --classpath "$(nbb script/nbb-classpath.cljk)" script/tests-on-nbb.cljk` (evidence/test-nbb-0657.out, NBB_EXIT=0) — 同一カウント 357 tests / 915 assertions, 0 failures。**注意 (既知 pitfall 再確認 0657)**: classpath なしの `nbb script/tests-on-nbb.cljk` は `Could not find namespace: torihiki.address-test` で落ちる (この反復で 1 度失敗して classpath 付きで再実行)。classpath は必ず script/nbb-classpath.cljk 経由。
