...
- **テスト両 runtime 実測 29 度目**: JVM `clojure -M:test` (evidence/test-0657.out, JVM_EXIT=0) と nbb `nbb --classpath "$(nbb script/nbb-classpath.cljs)" script/tests-on-nbb.cljs` (evidence/test-nbb-0657.out, NBB_EXIT=0) — 同一カウント 357 tests / 915 assertions, 0 failures。**注意 (既知 pitfall 再確認 0657)**: classpath なしの `nbb script/tests-on-nbb.cljs` は `Could not find namespace: torihiki.address-test` で落ちる (この反復で 1 度失敗して classpath 付きで再実行)。classpath は必ず script/nbb-classpath.cljs 経由。
- **seeded fuzz digest 26 度目**: JVM `clojure -M -e '(load-file "evidence/fuzz-seeded.cljc") (fuzz-seeded/run)'` (evidence/fuzz-jvm-0657.out) vs nbb driver (evidence/fuzz-nbb-driver.cljs → evidence/fuzz-nbb-0657.out) — REPL echo 行 (#'fuzz-seeded/run) を除き byte-identical、かつ 0608 baseline とも byte-identical (diff evidence/fuzz-jvm-0657.out evidence/fuzz-jvm-0608.out = 0)。JVM 呼び出しに `(fuzz-seeded/run)` が無いと echo 行のみで走らない (fuzz-seeded.cljc 先頭の呼び出しコメントがこれを反映していない件、引き続き注意)。
- **bench-tape-cancel-arity 20 実行目の再確認**: `clojure -M:bench` RED (EXIT=1), ArityException `Wrong number of args (2) passed to: torihiki.book/cancel!` at bench.clj:112 (evidence/bench-0657.err)。bench.clj への修正 commit なし (last touch a279427)。
- **validate-i53-halt fix 未着陸**: src/torihiki/api.cljc の :bad-amount 検査は integer?/pos? のまま i53 上限なし (grep 確認 0657)。
