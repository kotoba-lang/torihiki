...
3. seeded fuzz 16 seeds 再実行:
   - JVM: `kbb -M -e "$(cat evidence/fuzz-seeded.cljk) (fuzz-seeded/run)"` → evidence/fuzz-jvm-2111.out
   - nbb: `kbb --backend sci --classpath "$(kbb --backend sci script/nbb-classpath.cljk)" -e "..."` → evidence/fuzz-nbb-2111.out
...
harness を `kbb -M:test` と `script/tests-on-nbb.cljk` に接続して常設化する
